			...
			for (int x = 0; x < sizeX; x++) {
				if (data[z * sizeY * sizeX + y * sizeX + x]) {
					...
					if (x != 0) { //left
						...
						if (!neighborChunks[3]->IsRealNeighbor(posX, posY) || !neighborChunks[3]->isFilled(sizeX - 1, y, z)){
							faces |= LEFT;
						}
					}
					...
					if (x != sizeX - 1) { //right
						...
						if (!neighborChunks[1]->IsRealNeighbor(posX, posY) || !neighborChunks[1]->isFilled(0, y, z)){
							faces |= RIGHT;
						}
					}
					...
					if (y != 0) { //back
						...
						if (!neighborChunks[2]->IsRealNeighbor(posX, posY) || !neighborChunks[2]->isFilled(x, sizeY - 1, z)){
							faces |= BACK;
						}
					}
					...
					if (y != sizeY - 1) { //front
						...
						if (!neighborChunks[0]->IsRealNeighbor(posX, posY) || !neighborChunks[0]->isFilled(x, 0, z)){
							faces |= FRONT;
						}
					}
